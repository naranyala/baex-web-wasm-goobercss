import { ExbaComponent } from '../../framework/core/component';
import { t } from '../../styles';

const STYLES = `
  .container {
    padding: 2rem;
    display: flex;
    flex-direction: column;
    align-items: center;
    color: ${t.zinc100};
    font-family: inherit;
  }
  .profile-card {
    background: ${t.zinc800a};
    border: 1px solid ${t.zinc700};
    border-radius: 1.5rem;
    padding: 2rem;
    text-align: center;
    max-width: 400px;
    width: 100%;
    box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.3);
  }
  .avatar {
    width: 80px;
    height: 80px;
    background: ${t.indigo600};
    border-radius: 50%;
    margin: 0 auto 1rem;
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 2rem;
    color: ${t.white};
  }
  .name {
    font-size: 1.25rem;
    font-weight: 600;
    margin-bottom: 0.25rem;
    color: ${t.zinc200};
  }
  .role {
    font-size: 0.875rem;
    color: ${t.zinc400};
    margin-bottom: 1.5rem;
  }
  .info-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 1rem;
    text-align: left;
    border-top: 1px solid ${t.zinc700};
    padding-top: 1.5rem;
  }
  .info-item {
    display: flex;
    flex-direction: column;
    gap: 0.25rem;
  }
  .info-label {
    font-size: 0.75rem;
    text-transform: uppercase;
    color: ${t.zinc500};
    letter-spacing: 0.05em;
  }
  .info-value {
    font-size: 0.875rem;
    color: ${t.zinc300};
  }
`;

export class ProfileComponent extends ExbaComponent {
  static props = {
    username: 'string',
    role: 'string',
    location: 'string',
    joined: 'string',
    status: 'string',
    level: 'string',
  };

  static styles = STYLES;

  render() {
    const {
      username = 'Developer User',
      role = 'Fullstack Engineer @ EXBA',
      location = 'Remote',
      joined = 'Jun 2026',
      status = 'Active',
      level = 'Pro',
    } = this.state;

    return `
      <div class="container">
        <div class="profile-card">
          <div class="avatar">👤</div>
          <div class="name">${username}</div>
          <div class="role">${role}</div>
          <div class="info-grid">
            <div class="info-item">
              <span class="info-label">Location</span>
              <span class="info-value">${location}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Joined</span>
              <span class="info-value">${joined}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Status</span>
              <span class="info-value" style="color: ${t.emerald400}">${status}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Level</span>
              <span class="info-value">${level}</span>
            </div>
          </div>
        </div>
      </div>
    `;
  }
}

customElements.define('exba-profile', ProfileComponent);
