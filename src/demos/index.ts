    ...
    action: () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (openBtn && overlay) {
          ...
          overlay.onclick = (e: any) => { if (e.target === overlay) overlay.style.display = 'none'; };
        }
      });
    }
  },
    ...
    action: () => {
      ...
      function showToast(message: string, type: string) {
        ...
        Object.assign(el.style, {
          padding: '0.75rem 1.25rem', borderRadius: 'var(--radius-md)', background: bg,
          color: 'white', fontSize: '0.875rem', fontWeight: '500', boxShadow: '0 8px 24px rgba(0,0,0,0.4)',
          transition: 'opacity 0.3s, transform 0.3s', opacity: '0', transform: 'translateX(1rem)',
          pointerEvents: 'auto'
        });
      ...
      requestAnimationFrame(() => {
        view.querySelector('#toast-success')?.addEventListener('click', () => showToast('Operation completed successfully!', 'success'));
        view.querySelector('#toast-error')?.addEventListener('click', () => showToast('Something went wrong. Please try again.', 'error'));
        view.querySelector('#toast-info')?.addEventListener('click', () => showToast('Here is some useful information.', 'info'));
      });
    }
  },
  {
      ...
      requestAnimationFrame(() => {
        ...
        view.querySelector('#progress-start')?.addEventListener('click', () => {
          ...
          interval = setInterval(() => {
            ...
            if (width >= 100) { clearInterval(interval); interval = null; }
          }, 60);
        });
        ...
        view.querySelector('#indeterminate-toggle')?.addEventListener('click', () => {
          const bar = view.querySelector('#progress-indeterminate') as HTMLElement;
          bar.style.animation = bar.style.animation ? 'none' : 'indeterminate 1.5s ease-in-out infinite';
        });
        const style = document.createElement('style');
        style.textContent = '@keyframes indeterminate { 0% { transform: translateX(-100%); } 100% { transform: translateX(350px); } }';
