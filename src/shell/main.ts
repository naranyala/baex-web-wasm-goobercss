...
window.addEventListener('error', (event) => {
  console.error('[EXBA Global Error]', event.error);
  const overlay = document.createElement('exba-error-overlay');
  overlay.setAttribute('title', 'Runtime Error');
  overlay.setAttribute('message', event.message || 'An unexpected error occurred.');
  overlay.setAttribute('stack', event.error?.stack || '');
  document.body.appendChild(overlay);
});

window.addEventListener('unhandledrejection', (event) => {
  console.error('[EXBA Promise Rejection]', event.reason);
  const overlay = document.createElement('exba-error-overlay');
  overlay.setAttribute('title', 'Promise Rejection');
  overlay.setAttribute('message', event.reason?.message || 'An async operation failed unexpectedly.');
  overlay.setAttribute('stack', event.reason?.stack || '');
  document.body.appendChild(overlay);
});
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    router.register({ path: '/dashboard', component: 'exba-dashboard' });
    ...
    router.register({ path: '/sqlite-explorer', component: 'exba-sqlite-explorer' });
