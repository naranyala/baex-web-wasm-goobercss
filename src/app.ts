...
import { tabs, state, saveTabState, restoreTabState, renderTabBar } from './tab-state.js';
...
export async function initApp() {
  ...
  tabBar?.addEventListener('tab-close', (e: any) => {
    ...
    if (tabs.size === 0) {
      ...
      if (tab) {
        ...
        (document.getElementById('dynamic-view') as HTMLElement).style.display = 'block';
        tab.action();
      }
    }
    renderTabBar();
    ...
    if (tables.length === 0) {
      ...
      tables.forEach((tableName: string) => {
        ...
        el.onclick = () => {
          ...
          tabs.set(tabId, { label: tableName, action: () => tableOps.renderTableContent(tableName) });
  ...
  demoRegistry.forEach(demo => addDemoItem(demo.icon, demo.label, demo.id, demo.action));
  ...
  if (saved) {
    ...
    state.activeTabId = saved.activeTabId && tabs.has(saved.activeTabId) ? saved.activeTabId : null;
  }
